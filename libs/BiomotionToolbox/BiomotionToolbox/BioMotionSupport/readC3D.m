...
% JvB commented out because if was causing dysfunction,
% Matlab 2008a and up do not support VAXD anymore
if proctype==2,   
   fclose(fid);
   warning('readC3D:wrongformat','readC3D: The file was identified as having VAX format, which is not supported. Will try to read it in native format, which may allow you to use the file nonetheless.'); %% JvB added
   %fid=fopen(FullFileName,'r','d'); % DEC VAX D floating point and VAX ordering
   fid=fopen(FullFileName,'r','n');
end
